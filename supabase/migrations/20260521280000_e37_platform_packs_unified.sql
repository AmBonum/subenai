...
-- ============================================================================
-- Plan:   tasks/PLAN-2026-05-20-E37-tests-coverage.md (Phase A3 → Phase B')
-- Source: E37_SEED.sql (salvaged via PR #111). This file is the canonical
--         migration; the seed remains in repo root for ops convenience but
--         the diff between the two is just this header.
--
-- This single migration encodes what was originally planned as Phases B+C+D+E:
--   B: public.platform_pack_metadata table + sources_jsonb on questions
--      + get_platform_packs() / get_pack_with_questions() RPCs
--   C: 30 new questions across 6 packs (heslo-2fa, ai-deepfake,
--      socialne-siete, rodicia, skoly, zdravotnictvo)
--   D: 9 existing static packs materialised into public.tests +
--      platform_pack_metadata + test_questions
--   E: 6 new packs materialised the same way
--
-- Why mega-migration and not 4 separate timestamps:
--   - The salvaged seed already encodes consistent boundaries; re-splitting
--     would introduce review surface area without information gain.
--   - One paste = one verify; partial application is impossible (Phases D
--     and E NO-OP cleanly if the platform user is absent — see below).
--
-- OPERATIONAL PREREQUISITE (Phases D + E only):
--   Create the platform-system user via Supabase Dashboard BEFORE running
--   this migration:
--     Authentication → Users → Add user
--     Email:        platform@subenai.sk
--     Auto Confirm: ✓
--     Password:     any strong value (no human login flow)
--   If absent, Phases B + C still apply normally; D + E RAISE NOTICE and
--   NO-OP. Re-apply this migration after creating the user — B + C are
--   idempotent no-ops on the second run, D + E will then complete.
--
-- Why Dashboard creation, not INSERT INTO auth.users:
--   auth.users is Supabase-managed schema with companion rows
--   (auth.identities, auth.flow_state) and a version-dependent column list.
--   Migration-level inserts are fragile and silently miss the companions.
--   One-time Dashboard creation is the same pattern the team uses for the
--   audit test user (CLAUDE.md).
--
-- Idempotency: every CREATE uses IF NOT EXISTS / OR REPLACE, every INSERT
-- uses ON CONFLICT DO NOTHING. Re-applying is a safe no-op.
-- ============================================================================
