...
- **Žiadosti o vymazanie (Art. 17) sa po novom dokončia automaticky (E46.5).**
  ...
  s možnosťou zrušiť. Po uplynutí okna sa **doteraz nič nestávalo** — riadok
  zostal v queue, kým niekto manuálne nedotiahol vymazanie cez SQL. Od dnes
  beží Postgres cron `pending-erasures-flush` každú minútu — vypichne všetky
  riadky po grace window-e, vykoná `DELETE FROM auth.users` (cascade cez
  všetky súvisiace tabuľky), označí riadok ako spracovaný a zapíše forenzný
  audit log záznam. Maximálne 6 minút od potvrdenia po skutočné vymazanie.
  Snapshot pred vymazaním ostáva uložený natrvalo (GDPR Art. 5 ods. 2 —
  povinnosť rekonštruovateľnosti). Pre používateľa žiadna viditeľná zmena;
  pre operátora významné zlepšenie SLA spoľahlivosti.
