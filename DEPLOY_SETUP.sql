...
-- =============================================================================
-- supabase/migrations/20260522140000_e48_regenerate_view_token.sql
...
-- E48-v3 — Multi-assignee tickets + junction table + 3 RPCs + view + inline
-- signed-URL variant. Drops legacy support_tickets.assigned_to column.
-- =============================================================================
...
-- Audit trigger (defense-in-depth; direct table writes via service_role still log)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    INSERT INTO public.audit_log (
      ...
      jsonb_build_object('assignee_id', NEW.user_id, 'assignee_email', v_assignee_email, 'assigned_by', NEW.assigned_by),
      now()
    );
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    ...
    INSERT INTO public.audit_log (
      ...
      jsonb_build_object('assignee_id', OLD.user_id, 'assignee_email', v_assignee_email),
      now()
    );
    RETURN OLD;
  END IF;
...
-- Backfill from legacy assigned_to before dropping the column
...
-- Replace immutability+audit trigger so it no longer references assigned_to
DROP TRIGGER IF EXISTS support_tickets_immutability_and_audit_trg ON public.support_tickets;
  ...
  IF NEW.status IS DISTINCT FROM OLD.status
    ...
    INSERT INTO public.audit_log (
      ...
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'deleted_at', CASE WHEN NEW.deleted_at IS DISTINCT FROM OLD.deleted_at THEN jsonb_build_object('from', OLD.deleted_at, 'to', NEW.deleted_at) END,
        'archived_at', CASE WHEN NEW.archived_at IS DISTINCT FROM OLD.archived_at THEN jsonb_build_object('from', OLD.archived_at, 'to', NEW.archived_at) END,
        'view_token_invalidated_at', CASE WHEN NEW.view_token_invalidated_at IS DISTINCT FROM OLD.view_token_invalidated_at THEN jsonb_build_object('from', OLD.view_token_invalidated_at, 'to', NEW.view_token_invalidated_at) END
      )),
      now()
    );
  END IF;
...
-- assign_admin_to_ticket(uuid, uuid) -> jsonb
CREATE OR REPLACE FUNCTION public.assign_admin_to_ticket(p_ticket_id uuid, p_user_id uuid)
RETURNS jsonb
...
BEGIN
  ...
  END IF;
  SELECT EXISTS (SELECT 1 FROM public.support_tickets WHERE id = p_ticket_id AND deleted_at IS NULL) INTO v_ticket_exists;
...
-- unassign_admin_from_ticket(uuid, uuid) -> jsonb
CREATE OR REPLACE FUNCTION public.unassign_admin_from_ticket(p_ticket_id uuid, p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, extensions, pg_temp
AS $$
...
BEGIN
  ...
  IF v_aal IS DISTINCT FROM 'aal2' THEN
    RAISE EXCEPTION 'not_authorized: aal2 required';
  END IF;
  DELETE FROM public.support_ticket_assignees WHERE ticket_id = p_ticket_id AND user_id = p_user_id;
  GET DIAGNOSTICS v_deleted = ROW_COUNT;
  IF v_deleted > 0 THEN
    UPDATE public.support_tickets SET updated_at = now() WHERE id = p_ticket_id;
  END IF;
  RETURN jsonb_build_object('ticket_id', p_ticket_id, 'user_id', p_user_id, 'removed', v_deleted > 0);
END;
$$;
...
-- request_attachment_signed_url — extended with p_inline boolean DEFAULT false.
-- Drops legacy single-arg overload so PostgREST resolves unambiguously.
...
AS
SELECT
  t.id, t.created_at, t.updated_at, t.status, t.category, t.source,
  t.subject, t.body, t.submitter_user_id, t.submitter_email, t.submitter_name,
  t.archived_at, t.deleted_at, t.resolved_at,
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object(
