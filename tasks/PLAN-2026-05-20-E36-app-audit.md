...
| ~~B5~~ | ~~Error-state tests~~ ✅ — 5 tests in new `error-states.spec.ts` covering 500 on `tests` table, 500 on `respondent_groups`, 403 RLS denial on `notifications`, 404 missing detail (`/app/tests/<missing-id>`), and a multi-table failure. The contract: shell + page header + sidebar nav stay mounted; hooks degrade to their `?? []` empty fallback (no white screen, no React error boundary surfacing). | M | new `error-states.spec.ts` |
