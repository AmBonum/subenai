...
// E37 Phase G (2026-05-21) — test packs are DB-only. The static
// src/content/test-packs/ layer has been removed; pack URLs only
// appear in the sitemap when Supabase credentials are present at
// build time. Preview builds without DB creds silently skip pack URLs.
async function loadPublishedPackSlugs() {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.warn("[sitemap] No Supabase creds; skipping pack URLs (DB-only post-E37 Phase G)");
    return [];
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[sitemap] get_platform_packs RPC returned ${res.status}; skipping pack URLs`);
      return [];
    }
    const rows = await res.json();
    return rows.map((r) => ({ slug: r.slug, lastmod: r.published_at?.slice(0, 10) ?? TODAY }));
  } catch (err) {
    console.warn(`[sitemap] get_platform_packs RPC failed: ${err.message}; skipping pack URLs`);
    return [];
  }
}

const packs = await loadPublishedPackSlugs();
