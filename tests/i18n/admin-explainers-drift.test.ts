...
// Mirrors the nav-key list AdminSidebar emits for /admin/* routes.
const EXPECTED_KEYS = [
  "dashboard",
  "tests",
  "quick_test",
...
const skExplainers = (skAdmin as unknown as { explainers?: Json }).explainers;
const enExplainers = (enAdmin as unknown as { explainers?: Json }).explainers;
...
describe("admin explainers — i18n shape (SK)", () => {
  ...
  it.each([...EXPECTED_KEYS])(
    "'%s' — docs.links is a non-empty array of {label, href} with /docs/admin/* hrefs",
    (key) => {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        expect(
          (link.href as string).startsWith("/docs/admin/"),
          `href at ${key}.docs.links[${i}] must start with /docs/admin/ (got ${String(link.href)})`,
        ).toBe(true);
      }
    },
  );
...
describe("admin explainers — locale parity (sk vs en vs cs)", () => {
  it("EN admin bundle includes the explainers subtree", () => {
    expect(enExplainers, "explainers subtree missing from en/admin.json").toBeDefined();
  });

  it("CS admin bundle includes the explainers subtree", () => {
    expect(csExplainers, "explainers subtree missing from cs/admin.json").toBeDefined();
  });
